...
// Initialize ASCII text effect
function initASCIIText(containerId, options = {}) {
    const {
        text = 'Timagotchi',
        asciiFontSize = 10,
        textFontSize = 200,
    ...
    const createAndInit = async (containerElem, w, h) => {
            ...
            observer = new IntersectionObserver(
                async ([entry]) => {
                    if (entry.isIntersecting && entry.boundingClientRect.width > 0 && entry.boundingClientRect.height > 0) {
                        ...
                        asciiInstance = await createAndInit(container, w, h);
                        if (asciiInstance) {
                            asciiInstance.load();
                        }
                    }
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        if (document.getElementById('ascii-text-container')) {
            initASCIIText('ascii-text-container');
        }
    });
} else {
    if (document.getElementById('ascii-text-container')) {
